...
def captures(request, id_x):
    cam = get_object_or_404(Cameras, pk=id_x)
    response = s3.list_objects_v2(Bucket='app-bucket-8283', Prefix='Accident/')
...
def manual(request, id_x):
    cam = get_object_or_404(Cameras, pk=id_x)
    response = s3.list_objects_v2(Bucket='app-bucket-8283', Prefix='/Manual')
    objects = []
    if 'Contents' in response:
        for obj in response['Contents']:
            url = s3.generate_presigned_url(
    ...
    return render(request, "manual/manual.html",{"data_cam":cam,'objects': objects})
